const Userlog = (props) => {
    ...
    return (
        <div id="userslog" className="absolute left-0 w-[37.5vw] h-[100vh] bg-white overflow-y-scroll rounded-lg ">
            ...
            {roomId ? <header className="text-center w-[full] text-center pt-[15px] ">Active users:</header> : ""}
            <ul className="w-full h-[77.5vh] overflow-y-scroll overflow-x-hidden">
                {users.map((users, index) => (
                    <li className="rounded-lg w-full text-black break-words max-w-[100%] " key={index}>{`${users}`}</li>
                ))}
            </ul>
